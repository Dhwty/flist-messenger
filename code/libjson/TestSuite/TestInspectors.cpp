...
void TestSuite::TestInspectors(void){
    UnitTest::SetPrefix("Inspectors");
		  ...
		  json_free(res);
		  assertEquals(json_as_int(test), 0);
		  assertEquals(json_as_float(test), 0.0f);
		  ...
		  json_free(res);
		  assertEquals(json_as_int(test), 1);
		  assertEquals(json_as_float(test), 1.0f);
		  assertEquals(json_as_bool(test), true);
		  ...
		  TEST_PARSING_ITSELF(test);
		  
		  JSONNODE * casted = json_as_array(test);
		  UNIT_TEST(assertNotEquals(((JSONNode*)casted) -> internal, ((JSONNode*)test) -> internal);)
		  assertEquals(json_type(casted), JSON_ARRAY);
		  assertEquals(json_type(test), JSON_NODE);
		  assertEquals(json_size(test), 3);
		  assertEquals(json_size(casted), 3);
		  TEST_PARSING_ITSELF(casted);
		  
		  UnitTest::SetPrefix("Location");
		  ...
		  assertNull(json_get(test, JSON_TEXT("meh")));
			 ...
			 if(char * bin = (char*)json_as_binary(test, &i)){
				char * terminated = (char*)memcpy(malloc(i + 1), bin, i);
				terminated[i] = '\0';
		  ...
		  UNIT_TEST(assertNotEquals(casted.internal, test.internal);)
		  ...
		  assertEquals(test[JSON_TEXT("hi")], JSON_TEXT("world"));
		  assertEquals(test[JSON_TEXT("hello")], JSON_TEXT("mars"));
		  assertEquals(test[JSON_TEXT("salut")], JSON_TEXT("france"));
